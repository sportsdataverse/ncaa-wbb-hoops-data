...
This repo has NO R dataset chain -- it is a pure Python producer -- so there
is nothing to pair across languages. The contract here is Python-internal:
the ``REGISTRY`` dict in ``config.py``, the numbered shims beside the
package, and the stage numbers must agree.
...
Portability: this engine derives the family prefix from the registry's own
tag scheme and imports the package by its discovered name, so this file is
byte-identical between the NCAA MBB and WBB twins. Any diff between the two
copies is drift, not a league difference.
"""
...
def _registry() -> dict:
    """Import the package's own ``config.REGISTRY`` -- the thing the build iterates."""
    hits = sorted(_package_dir().rglob("config.py"))
    assert hits, f"no config.py under {_package_dir()}"
    assert len(hits) == 1, (
        f"expected exactly one config.py under {_package_dir()}, "
        f"found {[str(p.relative_to(REPO)) for p in hits]}"
    )
    mod = importlib.import_module(f"{_package_dir().name}.config")
    assert hasattr(mod, "REGISTRY"), f"{hits[0]} has no REGISTRY"
    return mod.REGISTRY
...
def _family() -> str:
    """Stage-file prefix, e.g. ``ncaa_mbb`` -- recovered from the registry's own
    tag scheme (each ``DatasetSpec.tag`` is ``f"{family}_{key}"`` by construction).
    """
    registry = _registry()
    families = set()
    for key, spec in registry.items():
        suffix = f"_{key}"
        assert spec.tag.endswith(suffix), (
            f"{key}: tag {spec.tag!r} does not end with {suffix!r}"
        )
        families.add(spec.tag[: -len(suffix)])
    assert families, "registry is empty"
    assert len(families) == 1, f"registry mixes families {sorted(families)}"
...
def _shims() -> dict[str, tuple[str, Path]]:
    ...
    pattern = re.compile(
        rf"^{re.escape(_family())}_(?P<num>\d{{2}})_(?P<key>.+)_creation$"
    )
...
def test_stage_numbers_follow_registry_order():
    ...
    assert not out_of_order, (
        "stage numbers disagree with registry build order:\n"
        + "\n".join(
